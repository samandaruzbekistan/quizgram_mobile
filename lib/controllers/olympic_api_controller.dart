...
class OlympicApiController{
  ...
  Future<int> buyTurkishExam(int examId, String promocode) async {
    var user_id = box.get('id');
    var balans = box.get('balans');
    var token = box.get('token');
    var headers = {
    ...
    var request = http.MultipartRequest('POST', Uri.parse(WebApiConstans.buyTurkishExam));
    request.fields.addAll({
      'user_id': '${user_id}',
      'exam_day_id': "${examId}",
      'promocode': '${promocode}'
  ...
  Future<int> buyNationalExam(int examId, String promocode) async {
    var user_id = box.get('id');
    var balans = box.get('balans');
    var token = box.get('token');
    var headers = {
    ...
    var request = http.MultipartRequest('POST', Uri.parse(WebApiConstans.buyNationalExam));
    request.fields.addAll({
      'user_id': '${user_id}',
      'exam_day_id': "${examId}",
      'promocode': '${promocode}'
